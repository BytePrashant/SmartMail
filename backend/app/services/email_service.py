...
class EmailService:
    ...
    def send_single_email(self, to_email: str, subject: str, body: str) -> EmailStatus:
        """Send a single email and return its status."""
        status = EmailStatus(
            recipient=to_email,
            status="pending",
            sent_at=None
    ...
    def send_batch_emails(
        ...
        Args:
            emails: List of dictionaries containing 'email', 'subject', and 'body'
            max_per_run: Maximum number of emails to send in this batch (defaults to settings.MAX_EMAILS_PER_RUN)
        
        Returns:
            EmailBatchResponse with results and statistics
        """
        if not emails:
